...
class EventFilterForm(MagiFiltersForm):
    ...
    def boost_members_to_queryset(self, queryset, request, value):
        return queryset.filter(boost_members=value)

    boost_members = forms.ChoiceField(choices=BLANK_CHOICE_DASH + [(id, full_name) for (id, full_name, image) in getattr(django_settings, 'FAVORITE_CHARACTERS', [])], initial=None, label=_('Boost Member'))
    boost_members_filter = MagiFilter(to_queryset=boost_members_to_queryset)
