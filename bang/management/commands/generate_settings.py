...
def generate_settings():
    ...
    cards = models.Card.objects.exclude(Q(art__isnull=True) | Q(art='')).exclude(i_rarity=1).exclude(show_art_on_homepage=False, show_trained_art_on_homepage=False).filter(member__i_band=models.Member.get_i('band', 'Roselia')).order_by('-release_date')[:200]
    homepage_cards = []
    for c in cards:
        print c
